...
func main() {
	...
	backend := router.Group("/backend")
	{
		...
		backend.PUT("/threads/:id", putThread)
		backend.DELETE("/threads/:id", deleteThreadByID)
...
func putThread(c *gin.Context) {
	...
	threadID, err := replaceThread(id, newThread)
	if err != nil {
		log.Fatal(err)
	}
	fmt.Printf("ID of added thread: %v\n", threadID)
...
func removeThreadByID(i int64) error {
	_, err := db.Exec("DELETE FROM thread WHERE id = ?", i)
	if err != nil {
...
func replaceThread(i int64, post Thread) (int64, error) {
	_, err := db.Exec("UPDATE thread SET id = ?, username = ?, title = ?, body = ?, time = ? WHERE id = ?", post.ID, post.Username, post.Title, post.Body, post.Time, i)
	if err != nil {
...
// no handler for this
/*
func removeRepliesByReplyID(i int64) error {
	_, err := db.Exec("DELETE FROM replies WHERE replyID = ?", i)
	if err != nil {
		return fmt.Errorf("removeThreadByID: %v", err)
	}
	return nil
